...
public:
	...
	virtual void go() {
		TP::array<unsigned char> buf(bufSize);
		while (true) {
			int nRead = read(0, &buf[0], bufSize);
			if (nRead == 0) break;
			for (int i = 0; i < nRead; ++i) dest.push(buf[i]);
		}
	}
};
...
public:
	...
	void push(unsigned char c) override {
...
public:
	typedef std::pair<unsigned char, size_t> item_type;
	...
	void push(const std::pair<unsigned char, size_t> & el) {
		while (el.first > c) {
			++c;
