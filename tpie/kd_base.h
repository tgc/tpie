...
public:
  ...
  memory_size_type max_intranode_height;
...
///////////////////////////////////////////////////////////////////////////
template<class coord_t, memory_size_type dim>
class kdtree_bin_node_base {
public:

  void initialize(const point<coord_t, dim> &p, memory_size_type d) {
    assert(d < dim);
    discr_val_ = p[d];
    discr_dim_ = d;
  }
  memory_size_type get_discriminator_dim() {
    return discr_dim_;
  }
  ...
  stream_offset_type &low_weight() {
    return lo_weight_;
  }
  stream_offset_type &high_weight() {
    return hi_weight_;
  }
  stream_offset_type low_weight() const {
    return lo_weight_;
  }
  memory_size_type high_weight() const {
    return hi_weight_;
  }
private:
  stream_offset_type lo_weight_;
  stream_offset_type hi_weight_;
#endif
...
public:
  ...
  void set_low_child(memory_size_type idx, link_type_t idx_type) {
    lo_child_ = idx;
    lo_type_ = idx_type;
  }
  void set_high_child(memory_size_type idx, link_type_t idx_type) {
    hi_child_ = idx;
    hi_type_ = idx_type;
  }
  void get_low_child(memory_size_type &idx, link_type_t &idx_type) const {
    idx = lo_child_;
    idx_type = lo_type_;
  }
  void get_high_child(memory_size_type &idx, link_type_t &idx_type) const {
    idx = hi_child_;
    idx_type = hi_type_;
  }
...
/// int instead of memory_size_type and link_type_t).
///////////////////////////////////////////////////////////////////////////
template<class coord_t, memory_size_type dim>
...
public:
  ...
  void get_low_child(memory_size_type &idx, link_type_t &idx_type) const {
    idx = (memory_size_type) lo_child_;
    idx_type = (link_type_t) lo_type_;
  }
  void get_high_child(memory_size_type &idx, link_type_t &idx_type) const {
    idx = (memory_size_type) hi_child_;
...
public:
  ...
  stream_offset_type high_weight() const {
    return hi_weight_;
  }
private:
  stream_offset_type lo_weight_;
...
public:
  ...
  region_t(const point<coord_t, dim>& p1, const point<coord_t, dim>& p2) {
    for (memory_size_type i = 0; i < dim; i++) {
  ...
  coord_t lo(memory_size_type d) const { return lo_[d]; }
  coord_t& lo(memory_size_type d) { return lo_[d];  }

  coord_t hi(memory_size_type d) const { return hi_[d]; }
  coord_t& hi(memory_size_type d) { return hi_[d];  }

  bool is_bounded_lo(memory_size_type d) const { return  (bd_[d] & LO_BD_MASK) != 0; }
  bool is_bounded_hi(memory_size_type d) const { return  (bd_[d] & HI_BD_MASK) != 0; }
  bool is_bounded(memory_size_type d) const 
  ...
  void set_bounded_lo(memory_size_type d, bool b) { bd_[d] |= (b ? LO_BD_MASK: 0); }
  void set_bounded_hi(memory_size_type d, bool b) { bd_[d] |= (b ? HI_BD_MASK: 0); }

  coord_t span(memory_size_type d) const { return hi(d) - lo(d); }
  ...
  /// coordinate.
  ///////////////////////////////////////////////////////////////////////////
  void cutout_hi(coord_t v, memory_size_type d) {
    hi_[d] = v;
    //    hi_bd_[d] = 1;//true;
    bd_[d] |= HI_BD_MASK;
  }
  ...
  // coordinate.
  ///////////////////////////////////////////////////////////////////////////
  void cutout_lo(coord_t v, memory_size_type d) {
    lo_[d] = v;
    //    lo_bd_[d] = 1;//true;
    bd_[d] |= LO_BD_MASK;
  }
  ...
  ///////////////////////////////////////////////////////////////////////////
  int relative_to_plane(coord_t sp, memory_size_type d) const {
...
struct path_stack_item_t {
  ...
  path_stack_item_t(const kdb_item_t<coord_t, dim>& ki, memory_size_type di, 
		    memory_size_type idx = 0): item(ki), d(di), el_idx(idx) {}
  path_stack_item_t() {}
};
